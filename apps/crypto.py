...
def app():
    ...
    * **This Project was done by 20pd32 and 20pd25**
    ...
    num_coin = col1.slider("Display Top N Coins", 1, 15, 15)
    ...
    # col2.dataframe(df_coins.set_index('coin_symbol').drop(['market_cap'],axis=1))
    col2.table(df_coins.set_index('coin_symbol').drop(['market_cap'],axis=1))
    ...
    df_change["+ve_change_1h"] = df_change["percent_change_1h"] > 0
    df_change["+ve_change_24h"] = df_change["percent_change_24h"] > 0
    df_change["+ve_change_7d"] = df_change["percent_change_7d"] > 0
    col2.dataframe(df_change,width=9000,height=570)
